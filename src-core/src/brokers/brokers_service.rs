...
impl BrokerDataService {
    pub async fn sync_all_accounts(conn: &mut SqliteConnection) -> Result<(), String> {
        ...
        // Load active accounts joined with enabled platforms
        // Only accounts with a linked platform that is enabled are considered
        let account_list: Vec<Account> = accounts
            .inner_join(crate::schema::platforms::table)
            .filter(is_active.eq(true))
            .filter(crate::schema::platforms::dsl::enabled.eq(true))
            .select((
                id,
                name,
                account_type,
            ...
            .load::<Account>(conn)
            .map_err(|e| format!("Failed to load accounts with enabled platforms: {}", e))?;

        debug!("Get account api list: {:?}", account_list);

        let asset_service = AssetService::new().await;
        for account in account_list {
            match BrokerProviderFactory::from_platform(&account).await {
                Ok(provider) => {
